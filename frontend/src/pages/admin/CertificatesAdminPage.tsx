            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead className="border-b border-border">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted uppercase tracking-wide">Issuer</th>
              ...
              <tbody className="divide-y divide-border">
                {certs.map((c) => (
                  <tr key={c._id} className="hover:bg-surface/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted text-sm">{c.issuer}</td>
                    <td className="px-4 py-3 text-muted text-xs">{formatDate(c.issueDate, 'month-year')}</td>
                    <td className="px-4 py-3">
